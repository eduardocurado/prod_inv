...
def calculate_adx(period, date_reference, length, coins=None):
    if not coins:
        coins = all_tickers
    for c in coins:
        ...
        adx = tb.ADX(df.high, df.low, df.close, timeperiod=length)

        try:
            momentum_entity = TechnicalIndicator(date_reference, period, c, 'ADX',  adx.iloc[-1])
            db.session.add(momentum_entity)
            db.session.commit()

        except Exception:
            db.session.rollback()
            print('Already has value ADX ' + str(date_reference))
